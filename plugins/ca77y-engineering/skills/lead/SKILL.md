...
When a host diagnostic or the user establishes that an inherited bad value is live in the current session, state that a settings-file edit does not change an already-running session. Direct the user to start a new session with the corrected configuration, then have its lead resume the pending dispatch from the ledger; retrying the same dispatch in the existing session is not a remedy.
