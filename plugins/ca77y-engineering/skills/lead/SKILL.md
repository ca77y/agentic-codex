...
When a host diagnostic or the user establishes that an inherited bad value is live in the current session, state this as a conditional operational assumption about that inherited value: a settings-file edit does not change that value in an already-running session. The dispatch contract exposes the requested inputs and host result, but provides no operation for this policy to reload or verify that inherited value. The [Codex configuration reference](https://developers.openai.com/codex/config-reference) documents model-catalog loading at startup and model defaults for new threads; those documented boundaries support the new-session remedy, while this policy makes no claim about other settings or an undocumented live-reload mechanism. Direct the user to start a new session with the corrected configuration, then have its lead resume the pending dispatch from the ledger; retrying the same dispatch in the existing session is not a remedy.
