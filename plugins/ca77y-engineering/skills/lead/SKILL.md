...
**Dispatch first-class custom subagents by configured name**: `ca77y_engineering_writer`, `ca77y_engineering_junior_coder`, `ca77y_engineering_senior_coder`, `ca77y_engineering_qa`, and `ca77y_engineering_auditor`. Each installed TOML embeds that worker's complete role procedure and references; no worker role is a skill, and you never perform it in the main session. You select the model and reasoning effort per *The `--fast` flag*. A listed agent name proves only catalog availability, not that its installed TOML matches this plugin's current agent definitions. After an engineering-plugin update that changes `agents/` or the `install-subagents` resources, require the user to run `ca77y-engineering:install-subagents` from the updated plugin and start a new Codex task before the first dispatch. The refresh gate is durable: before the first dispatch in any session, including a replacement task, read the ledger's **agent-definition state**. The invoking task follows `install-subagents`'s sequence of `--check`, installation with `--ledger-path <absolute-story-worktree>/tmp/ledger.md`, proof-write confirmation, and only then restart; that installer invocation produces the pre-restart portion. The replacement lead appends `post-install new-task confirmation` after opening the same worktree and ledger and before its first dispatch. The state records either a no-definition-change check tied to the last valid refresh proof, or an **agent-definition refresh proof** containing the updated plugin source revision, the absolute installer path, the successful `--check` and install result for every managed TOML, the managed agent names refreshed, and that post-install confirmation; verify that the recorded revision still covers the current `agents/` and `install-subagents` resources and that none changed after it. A new run with no prior proof cannot substitute a prompt-local acknowledgment or a no-change claim. If the state is absent, incomplete, stale, or cannot be reconciled with the current source, stop and request the refresh and a new task; do not dispatch a legacy definition. If a required name is unavailable, stop before substituting a generic worker and ask the user to run the same refresh. Record the returned agent id or canonical target; that target is how later rounds continue the same worker.
