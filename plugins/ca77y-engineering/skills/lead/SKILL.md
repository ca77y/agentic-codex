...
Worker access is **caller-granted, per dispatch**: for the **initial pre-build spec pass only**, the `writer` carries its own fixed read and search access plus whatever card-content authority `docs/BOARD.md` explicitly grants the writer; a later respec is read/search-only unless that declaration separately grants the later phase. The `auditor` gets **read and search** at the spec-readiness gate and **read** at the acceptance gate; the `coder` and `qa` get none. Board-side duplicate detection is theirs (the `writer`'s sibling sweep, the `auditor`'s readiness gate), never yours.
...
**Spawn.** Ask Codex to spawn the named custom subagent for a fresh worker. Give it a self-contained task, the spec path, absolute worktree path, provisioning status, relevant commit references, and granted board access. The custom-agent definition supplies the role instructions, model, and reasoning effort; do not replace it with a generic worker carrying a pasted role prompt. Record the returned target in the ledger.

**Resume or fresh — the routing rule for every round.** When findings go back to the `coder` or the `writer`, use `followup_task` on the recorded target if that worker is still available; otherwise spawn a **fresh** worker with the same role — and the same tier for the coder — carrying the spec path, worktree path, provisioning status, round commit references, and findings inline or by findings-file path. A coder follow-up also names its own `references/coder-fix-round.md`; a writer docs follow-up names its own `references/writer-docs-pass.md`. `qa` and the `auditor` are **never resumed**: every round is a fresh worker that re-reads the artifact on its own terms. *Route to the coder/writer per this section* means exactly this.
...
**`--fast` is the user's, arrives on the invocation, and changes exactly one thing: which configured custom-agent variant is dispatched.** The fast variants step the model down while preserving role instructions and reasoning effort:

| Role | Default custom agent | With `--fast` |
| --- | --- | --- |
| writer | `ca77y_engineering_writer` | `ca77y_engineering_writer_fast` |
| auditor | `ca77y_engineering_auditor` | `ca77y_engineering_auditor_fast` |
| junior coder | `ca77y_engineering_junior_coder` | unchanged |
| senior coder | `ca77y_engineering_senior_coder` | `ca77y_engineering_senior_coder_fast` |
| qa | `ca77y_engineering_qa` | `ca77y_engineering_qa_fast` |

**It steps the model and nothing else.** The build still routes on the **Coding complexity** score and a promotion still goes to the senior. The main session is unchanged, and the flag is not passed into worker prompts. Never turn it on or off yourself. Record the exact custom-agent name used for every dispatch.
...
**Land each transition where the visibility rule says**, so the human sees it now. Repo-local board: the repository root checkout on its base branch — one of the narrowly declared board-write exceptions to the read-only-root rule in *Addressing the story worktree*; the other is an explicitly authorised initial pre-build writer card correction. The story's own tree — code, spec, docs, dependencies — has no root-write exception. Hosted board: the bound call. **Never write a transition inside the story worktree; never commit, push, or branch for one**: leave a repo-local edit uncommitted in the root and never check the story branch out there. Record each transition in the ledger as you make it.
...
3. **Spec.** Spawn `ca77y_engineering_writer` (or its fast variant) to author the spec in the project's specs area — its *Acceptance criteria* section transcribes the card's `## Acceptance criteria` verbatim, `AC1`…`ACn`, taken **after** any criterion correction the spec pass makes on the card — and record its target. Then, in this order:
   ...
   - **Dispatch the gate.** Once the format step has run, been skipped, or been reported unrunnable, dispatch `ca77y_engineering_auditor` (or its fast variant) with **read and search** into the board: is the spec ready to build from? A mismatch on its equality check, or a duplicate, comes back as a blocking finding in its not-ready verdict.
...
4. **Build.** Spawn **one** configured coder with the spec path and worktree; trust what it reports it built and could not resolve. **Which coder is the spec's call, not yours**: read the **Coding complexity** score from the spec's metadata header — below 5 uses `ca77y_engineering_junior_coder`; 5 or above uses `ca77y_engineering_senior_coder` or its fast variant. **A score you cannot read — absent, unparseable, or outside 1–10 — routes to the senior custom agent**, recorded in the ledger with its reason. Record the score, tier, agent name, and worker target. Every later round goes to this same coder per *Dispatch, resume, and collection*; **the tier is fixed for the run**, changed only by a promotion under *The 3× rule*.
5. **Validate and review.** Commit the coder's build, then dispatch `ca77y_engineering_qa` (or its fast variant) to validate the build and review the diff. Route findings to the coder per *Dispatch, resume, and collection*. When its report arrives, commit the round, then dispatch a **fresh** QA custom subagent with the commit references and coder's fix report. Repeat until qa is clean, bounded by *The 3× rule*.
6. **Acceptance gate.** Commit anything the qa loop left uncommitted, including tests its last round added. Dispatch `ca77y_engineering_auditor` (or its fast variant) with **read** access to the board to verify the built result satisfies the spec's `AC1`…`ACn` transcription, naming its `references/auditor-acceptance-gate.md` as the file it reads first; every re-audit does the same. It performs the equality check itself before grading. Each criterion is one gate; each **unmet, partially met, or mis-worded** one is a finding named by its `ACn`. Name the spec, not the criteria. This is the **last gate you run**.
...
7. **Docs.** Dispatch a fresh `ca77y_engineering_writer` (or its fast variant) for the docs pass, naming its `references/writer-docs-pass.md` as the file it reads first. Hand it the **spec commit** and **round commit references** from the ledger. Trust what it returns; there is no docs gate.
