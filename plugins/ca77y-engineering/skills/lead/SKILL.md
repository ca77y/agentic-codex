...
**Dispatch first-class custom subagents by configured name**: `ca77y_engineering_writer`, `ca77y_engineering_junior_coder`, `ca77y_engineering_senior_coder`, `ca77y_engineering_qa`, and `ca77y_engineering_auditor`. Each installed TOML embeds that worker's complete role procedure and references; no worker role is a skill, and you never perform it in the main session. You select the model and reasoning effort per *The `--fast` flag*. A listed agent name proves only catalog availability, not that its installed TOML matches this plugin's current agent definitions. After an engineering-plugin update that changes `agents/` or the `install-subagents` resources, require the user to run `ca77y-engineering:install-subagents` from the updated plugin and start a new Codex task before the first dispatch. The refresh gate is durable: before the first dispatch in any session, including a replacement task, read the ledger's **agent-definition state**. The invoking task follows `install-subagents`'s sequence of `--check`, installation with `--ledger-path <absolute-story-worktree>/tmp/ledger.md`, proof-write confirmation, and only then restart; that installer invocation produces the pre-restart portion. Its source identity is the Git revision plus deterministic package digest when Git is available, or the package digest alone for a packaged copy without `.git`; no successful proof may contain an unavailable identity. The replacement lead opens the same worktree, runs the installer's read-only `--verify-ledger-path <absolute-story-worktree>/tmp/ledger.md`, and appends `post-install new-task confirmation` only after it passes and before its first dispatch. The state records either a no-definition-change check tied to the last valid refresh proof, or an **agent-definition refresh proof** containing the source identity, absolute installer path, successful `--check` and install result for every managed TOML, the managed agent names refreshed, and that post-install confirmation; verify that the recorded identity matches the current source before dispatch. A new run with no prior proof cannot substitute a prompt-local acknowledgment or a no-change claim. If the state is absent, incomplete, stale, unverifiable, or cannot be reconciled with the current source, stop and request the refresh and a new task; do not dispatch a legacy definition. If a required name is unavailable, stop before substituting a generic worker and ask the user to run the same refresh. Record the returned agent id or canonical target; that target is how later rounds continue the same worker.
...
**Dispatch artifact baseline — every spawn and resume.** Immediately before each `spawn_agent` or `followup_task`, record a dispatch id and a baseline in the ledger: the absolute worktree path; the `git -C <worktree> status --short --untracked-files=all` path/status set and a content fingerprint for every path in that set; and the existence and content fingerprint of every expected output path, including an ignored path the worker may write. Exclude the lead's ledger update itself from the comparison. After a no-report result, capture the same inventory. Attribute an artifact to that dispatch only when it is newly present or its fingerprint changed after the baseline; a pre-existing modified path or pre-existing artifact is not evidence that the failed worker did work. Carry the dispatch id and baseline into recovery and any escalation.

**Spawn.** Ask Codex to spawn the named custom subagent for a fresh worker with `fork_turns: "none"`, the role's model, and its reasoning effort from *The `--fast` flag*. Give it a self-contained task, the spec path, absolute worktree path, provisioning status, relevant commit references, and granted board access. The custom-agent definition supplies the complete role instructions, embedded references, and identity; do not paste them into the task or replace the role with a generic worker. Record the returned target, model, and effort in the ledger. If `spawn_agent` rejects synchronously before returning a target, record that no target exists and run the targetless recovery branch before classifying the dispatch; never invent a target or immediately retry. A target that is returned and later reaches an errored terminal state without a report uses the errored-target recovery branch. A replacement fresh worker uses the same spawn settings for the run; `followup_task` continues the already-selected model and effort.
...
A targetless rejection or a returned target that errored before reporting is not an ordinary unavailable target: complete the recovery and first-fault policy before any fresh worker dispatch.

**Collect.** Before waiting, update the ledger and send the user a concise progress note. Use `wait_agent` for completion notifications; never poll with shell commands or sleeps. The worker's final response is its report. If no usable report arrives, read `references/recovery.md` before any replacement dispatch. The recovery procedure also applies when `spawn_agent` returned no target and when a returned target is terminally errored; it checks the ledger baseline before deciding that no work was done. A user prompt mid-pipeline is a pause, not an abort: handle it, then resume from the ledger.

**Pre-work dispatch infrastructure fault.** Classify a dispatch as an infrastructure fault only after establishing all three conditions: (1) the fresh `spawn_agent` call or a `followup_task` delivery failed before the worker supplied a report of any work for that dispatch, or a target returned by either call later reached an errored terminal state without reporting; this includes a synchronous `spawn_agent` rejection before a target exists; (2) the recovery checks in `references/recovery.md` have ruled out a live worker, a delayed final report, and an attributable post-baseline worktree artifact; and (3) there is no worker report to route as a worker or gate problem. The worker's lack of any report is the boundary. A confirmed fault is recorded in the ledger separately from gate rounds, coder attempts, and the 3× counter, and consumes no attempt.

**First-fault diagnosis and session remedy.** On the first confirmed pre-work infrastructure fault, stop retrying it in the current session and escalate to the user with the requested custom-agent name, the model and reasoning effort supplied to `spawn_agent` (or the model and effort already recorded for the `followup_task` target), and the non-secret host error or returned target metadata. If the host error names model resolution, include that host-reported statement; any other host error does not establish a model-resolution or session-source cause. Include the host-reported effective model and whether its value came from the live session or disk when available; label each fact **unavailable** when the host does not expose it. Do not inspect `.env` files or assert a configuration-precedence cause. Include the dispatch id, the captured post-dispatch comparison, and the actual absolute handoff paths (substitute the story worktree path and its `<worktree>/tmp/ledger.md` path, never those placeholders). Tell the user that the replacement task must open that same worktree, read that ledger and `git log`, and continue the pending dispatch from its recorded state; it must not create a second worktree or rely on a prompt-only recollection. The escalation identifies the next action needed to start a clean session.

When a host diagnostic or the user establishes that an inherited bad value is live in the current session, state this as a conditional operational rule for that inherited value: absent an explicitly documented process-level reload, a settings-file edit alone does not change that value in an already-running session. [POSIX Issue 8](https://pubs.opengroup.org/onlinepubs/9799919799/functions/exec.html) specifies that `environ` is initialized for a new process image, that `envp` supplies that image's environment, and that the running application itself may update `environ`; this rule relies only on an external file edit not performing such an in-process update. It makes no claim about other settings or an undocumented Codex live-reload mechanism. Direct the user to start a new session with the corrected configuration, carrying the absolute worktree and `<worktree>/tmp/ledger.md` paths from the escalation, then have its lead verify the durable refresh proof and resume the pending dispatch from that ledger; retrying the same dispatch in the existing session is not a remedy.
...
**The ledger.** Maintain `tmp/ledger.md` from workspace creation onward: the task and card; the board's write authority; worktree path and provisioning status; current step; the durable **agent-definition state** (either the no-definition-change determination or refresh proof with source identity, absolute installer path, `--check` and install result for every managed TOML, managed names, and post-install new-task confirmation); whether `--fast` is in play and the model passed per dispatch; worker targets (writer, coder), including an explicit no-target result; the **Coding complexity** score and tier, any senior fallback with its reason, any promotion with what tripped it; round counters per gate; each gate's reported equality-check outcome; each dispatch id and its pre-dispatch artifact baseline; confirmed pre-work dispatch infrastructure faults separately from gate rounds, coder attempts, and the 3× counter, including their target status, dispatch diagnosis, absolute replacement paths, and session-remedy state; commits made; what is awaited; card transitions made, in the board's own values; retained board follow-ups. Update it **before every dispatch and before every wait or turn end.** After a compaction, wake, or session restart, the ledger plus `git log` — never recollection — say where the pipeline stands: read `references/recovery.md`.
