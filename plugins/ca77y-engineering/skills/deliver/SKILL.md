...
On a `spawn_agent` error or uncertain execution for any configured production or validation target, read [dispatch fault recovery](references/dispatch.md) before retrying or changing attempt accounting. Only host or target proof that assigned work never began permits a pre-work infrastructure classification and no solution-attempt charge. A report of assigned work retains normal evaluated-approach accounting; an error plus no report remains uncertain. Diagnose and escalate the first fault, recording the requested agent/model/effort and independently available host effective-model/source facts in the ledger; never blindly repeat the dispatch.
