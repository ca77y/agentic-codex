...
Read the request and acceptance source, relevant code/docs, current checkout and working changes, and any existing PR or prior verification. Reuse a suitable current environment and preserve unrelated work. Read `docs/BOARD.md` and `docs/FORGE.md` when their operations apply. Missing configuration blocks the corresponding operation, not already-authorized investigation or local preparation. Grants to legacy `analyst`, `writer`, or `lead` do not grant `deliver` write authority. Require explicit applicable bindings before writes; do not rewrite declarations to grant yourself authority.
