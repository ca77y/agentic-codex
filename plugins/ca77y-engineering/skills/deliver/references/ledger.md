...
Resolve the project's temp folder from its forge declaration (normally `docs/FORGE.md`). Resolve relative paths against the project root; preserve absolute paths. If the declaration or temp-folder setting is absent, use `.tmp/` under the project root without requiring bootstrap or the engineering plugin. Store project ledgers in `<temp-folder>/ledgers/`. Follow a supplied ledger path or a ledger reference in the task, spec, or PR when resuming existing work; otherwise search that directory for the same project identity, outcome, and acceptance source. Match project identity separately from the current checkout so entry-point or worktree changes do not create a new history. For work without a project, search `$CODEX_HOME/ledgers/` (default `~/.codex/ledgers/`). Use the actual project root, never the installed skill's working directory.
