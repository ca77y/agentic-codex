...
Follow a supplied ledger path or a ledger reference in the task, spec, or PR. Otherwise search the project's declared ledger directory, defaulting to `docs/ledgers/` under the project root, for the same initiating request/run, project identity, outcome, and acceptance source. The same PR or artifact alone is not a run match; link earlier runs as context without importing their attempt counts. Match project identity separately from the current checkout so entry-point or worktree changes do not create a new history. For work without a project, search `$CODEX_HOME/ledgers/` (default `~/.codex/ledgers/`). Use the actual project root, never the installed skill's working directory.
