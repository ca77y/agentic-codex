...
Explicit deliver invocation requests the existing PR repair endpoint. Push the verified correction and update that PR when the forge operation’s binding and conditions apply and the required gates pass; no separate publication request is needed. Follow the bound operations; avoid unrelated history changes. User authorization to implement does not by itself authorize comments or review-trigger messages to others. Report addressed findings, current verification, the existing PR identity, and any remaining blocked endpoint.
