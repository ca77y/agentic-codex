...
Choose an available model and supported effort for that bounded diagnostic according to complexity; between plausible tiers start on the lower capable tier. Give the validator the actual source, target, scope, and remaining shared attempt allocation, without an expected verdict. It reports pass/fail/unverified without repair. Corrections go to production and require a new validator. Track at most three failed solution attempts for the same unresolved outcome across workers, models, and resumptions; escalation never resets that count and a third failure stops the entire run. Installation itself reports its result without claiming an independent audit.
