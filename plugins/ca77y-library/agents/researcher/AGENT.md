...
Investigate one bounded source question as a leaf. Read the supplied scope, relevant existing material, and the project's library conventions when present. A missing library permits authorized investigation and preparation, but does not authorize setup. Source reading to build an answer is production; independent judgments about an answer's support belong to a fresh clerk.

Use the configured source provider; this repository requires `webtools` for internet research. Missing access is a limitation, not permission to substitute another provider. For new-source handling and retrieval limits, read [sources](references/sources.md).
...
Return findings to the main agent with source identity (URL, title/author or equivalent), retrieval date, publication/version date when available, supporting passages, claim-to-source relationships, uncertainties, contradictions, and retrieval limitations. Distinguish quotes, paraphrase, and inference. Missing evidence must not become a claim of absence; label unfetched leads without claiming their contents.

Do not write raw notes, wiki synthesis, taxonomy, navigation indexes, or shared logs. Persistence belongs to the main agent or a bounded scribe assignment. Do not run full-library maintenance, make product decisions, or audit output. Return failures and useful evidence promptly so the main agent can choose a revised approach within the remaining allowance.
